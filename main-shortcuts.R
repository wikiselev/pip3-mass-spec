...
bio_reps <- function() {
    ...
    for (c in unique(d1$Component.Name)) {
        p <- ggplot(d1[Component.Name == c], aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = as.factor(bio.rep))) + 
            geom_bar(position = "dodge", stat = "identity") + facet_grid(Genotype ~ Condition, scale = "free") + 
            theme_bw()
    ...
    d1 <- d1[, list(Golden.Ratio.Av.Tech.by.ctrl = Golden.Ratio.Av.Tech/Golden.Ratio.Av.Tech[Time == 1 & grepl("ctrl", 
        Condition)] * 100, Condition, Time), by = c("Component.Name", "Genotype", "Antagonist", "bio.rep")]
    d2 <- d2[, list(Golden.Ratio.Av.Tech.by.ctrl = Golden.Ratio.Av.Tech/Golden.Ratio.Av.Tech[Time == 1 & grepl("ctrl", 
        Condition)] * 100, Condition, Time), by = c("Component.Name", "Genotype", "Antagonist", "bio.rep")]
...
plot_species <- function() {
    ...
    for (c in unique(d$Component.Name)) {
        p <- ggplot(d[Component.Name == c], aes(x = as.factor(Time), y = Golden.Ratio, fill = Bio.Rep, color = Antagonist)) + 
            geom_boxplot() + facet_grid(Genotype ~ Condition, scale = "free") + theme_bw()
    ...
    d <- d[, list(Golden.Ratio.Av.Tech.by.ctrl = Golden.Ratio.Av.Tech/Golden.Ratio.Av.Tech[Time == 1 & grepl("ctrl", 
        Condition)] * 100, Condition, Time), by = c("Component.Name", "Genotype", "Antagonist", "Bio.Rep")]
    
    for (c in unique(d$Component.Name)) {
        p <- ggplot(d[Component.Name == c], aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech.by.ctrl, fill = Bio.Rep, 
            color = Antagonist)) + geom_bar(position = "dodge", stat = "identity") + geom_hline(yintercept = 100) + 
            facet_grid(Genotype ~ Condition, scale = "free") + theme_bw()
    ...
    for (f in files) {
        ...
        for (c in unique(d$Component.Name)) {
            p <- ggplot(d[Component.Name == c], aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = Antagonist)) + 
                geom_bar(position = "dodge", stat = "identity") + facet_grid(Genotype ~ Condition) + theme_bw()
            ggsave(p, file = paste0("../", name, "/", name, "-plot-species/", strsplit(f, "\\.")[[1]][1], 
                "/", c, ".pdf"), w = 8, h = 8)
        }
        # d <- d[,list(Golden.Ratio.Av.Tech.by.ctrl = Golden.Ratio.Av.Tech/Golden.Ratio.Av.Tech[Time == 1 &
        # grepl('ctrl', Condition)]*100, Condition, Time), by = c('Component.Name', 'Genotype', 'Antagonist')]
        # for(c in unique(d$Component.Name)) { p <- ggplot(d[Component.Name == c], aes(x = as.factor(Time), y =
        # Golden.Ratio.Av.Tech.by.ctrl, fill = Antagonist)) + geom_bar(position = 'dodge', stat='identity') +
        # geom_hline(yintercept = 100) + facet_grid(Genotype ~ Condition) + theme_bw() ggsave(p, file =
        # paste0('../', name, '/', name, '-plot-species-norm/', strsplit(f, '\\.')[[1]][1], '/', c, '.pdf'), w =
        # 8, h = 8) }
    }
}
...
process_data_inhibitors <- function(name) {
    ...
    data <- data[, c(3, 7, 11)]
    data$ISD <- 0
    data[data$Component.Name == "SA_P2_aldehyde_45", ]$ISD <- rep(data[data$Component.Name == "d6_SA_P2_aldehyde_45", 
        ]$Area, 1)
    data[data$Component.Name == "SA_P2_aldehyde_34", ]$ISD <- rep(data[data$Component.Name == "d6_SA_P2_aldehyde_34", 
        ]$Area, 1)
    data[data$Component.Name == "SA_PI_aldehyde", ]$ISD <- rep(data[data$Component.Name == "17:0-20:4_PI_aldehyde", 
        ]$Area, 1)
    data[data$Component.Name == "SA_PIP_aldehyde", ]$ISD <- rep(data[data$Component.Name == "17:0-20:4_PI_aldehyde", 
        ]$Area, 1)
    data <- data[data$ISD != 0, ]
    data$ng <- data$Area/data$ISD
    
    data$Golden.Ratio <- 0
    data[data$Component.Name == "SA_P2_aldehyde_45", ]$Golden.Ratio <- data[data$Component.Name == "SA_PI_aldehyde", 
        ]$ng
    data[data$Component.Name == "SA_P2_aldehyde_34", ]$Golden.Ratio <- data[data$Component.Name == "SA_PI_aldehyde", 
        ]$ng
    data[data$Component.Name == "SA_PI_aldehyde", ]$Golden.Ratio <- data[data$Component.Name == "SA_PI_aldehyde", 
        ]$ng
    data[data$Component.Name == "SA_PIP_aldehyde", ]$Golden.Ratio <- data[data$Component.Name == "SA_PI_aldehyde", 
        ]$ng
    data$Golden.Ratio <- data$ng/data$Golden.Ratio
    # import the sample information table in a text format. Please modify the sample iformation file in
    # advance so that it looks like (this is an example):
...
plot_species_inhibitors <- function() {
    ...
    for (c in unique(d$Component.Name)) {
        p <- ggplot(d[Component.Name == c], aes(x = Time, y = Golden.Ratio.Av.Tech, color = Antagonist, group = Antagonist)) + 
            geom_point() + geom_line() + facet_grid(Genotype ~ Condition) + theme_bw()
