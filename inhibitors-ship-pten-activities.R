...
d1 <- d[Component.Name == "SA_P2_aldehyde_34" & Antagonist == "-" & file != "2014_11_12B_c18_cIIa&cIIb_FULL"]

d2 <- d[Component.Name == "P3_4" & Antagonist == "-" & file != "2014_11_12A_c4_cIIa&cIIb_FULLn1"]
d.tot <- rbind(d1, d2)
...
p <- ggplot(d.tot, aes(Time, uM, group = Condition)) + geom_point(aes(shape = file)) + geom_line(aes(group = file)) + 
    facet_grid(Component.Name ~ Condition, scale = "free") + theme_bw() + labs(x = "Time, s", y = "PIP3, uM")
...
d3.S <- data.table()
for (t in c(0, 1, 5, 15)) {
    tmp <- d3[Component.Name == "P3_4" & (Time %in% 0:t), list(S = trapz(Time, uM) * 60 * 0.0138), by = c("file", 
        "Condition")]
    tmp$Time <- t
...
p <- ggplot(rbind(d3, d3.S), aes(Time, uM, group = Condition, color = cond)) + geom_point(aes(shape = file)) + 
    geom_line(aes(group = file)) + facet_grid(Component.Name ~ Condition, scale = "free") + theme_bw() + labs(x = "Time, s", 
    y = "PIP3, uM")
