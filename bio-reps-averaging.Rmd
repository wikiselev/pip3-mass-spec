...
rep10 <- get_bio_rep("2014_10_10A_c4_pabVSpabs", "P3_4")
rep10$rep <- "2014_10_10A_c4_pabVSpabs"
rep11 <- get_bio_rep("2015_02_14B_c4_pabVSpabsN2", "P3_4")
rep11$rep <- "2015_02_14B_c4_pabVSpabsN2"
...
```{r, results='asis'}
coef <- t[Time == 1 & Condition == "siINPP4(A+B)" & Genotype == "PTEN", list(norm.coef = 0.00779030920000628/Mean.GR), by = "rep"]
knitr::kable(coef)
```
...
write.csv2(d2, file = "all-data/bio-rep-av/p3_4.csv", row.names = F, quote = F)
...
Then I find a coefficient of normalisation for each replicate (so that it should be always 0.078066230364529 in PTEN-sictrl condition)

```{r, results='asis'}
coef <- d[Time == 1 & Condition == "sictrl" & Genotype == "PTEN", list(norm.coef = 0.078066230364529/Mean.GR), by = "rep"]
knitr::kable(coef)
```
...
```{r, fig.width=8, fig.height=4}
# remove the outlier
d <- d[rep != "2015_02_14A_c18_pten_ab_pab_tcN2_pabVSpabsN2"]
...
```{r}
# add last 2 experiments and normalise them by siINPP4(A+B) (there is no sictrl in them)
rep7 <- get_bio_rep("2014_10_10B_c18_pabVSpabs", "SA_P2_aldehyde_34")
rep7$rep <- "2014_10_10A_c4_pabVSpabs"
rep8 <- get_bio_rep("2015_02_14A_c18_pabVSpabsN2", "SA_P2_aldehyde_34")
rep8$rep <- "2015_02_14B_c4_pabVSpabsN2"
...
```{r, results='asis'}
coef <- t[Time == 1 & Condition == "siINPP4(A+B)" & Genotype == "PTEN", list(norm.coef = 0.146013940573659/Mean.GR), by = "rep"]
knitr::kable(coef)
```
...
write.csv2(d2, file = "all-data/bio-rep-av/pi34p2.csv", row.names = F, quote = F)
