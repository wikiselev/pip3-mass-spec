...
regr <- function(d, point.num, cond) {
	...
	print(summary(model))
	return(data.table(Time = d$Time[1:point.num],
		uM = exp(predict(model,list(Time=d$Time[1:point.num]))),
		Condition = cond))
}

d <- as.data.table(read.csv("../copasi/all-data-uM.csv"))

d1 <- d[Genotype == "PTEN" & Condition == "sictrl" &
		Antagonist == "1uMPI-103" & Component.Name == "P3_4" &
		file == "2014_12_18A_full"]
d1$Condition <- "PTEN"
t1 <- regr(d1, 5, "PTEN")
d2 <- d[Genotype == "WT" & Condition == "siSHIP2" &
		Antagonist == "1uMPI-103" & Component.Name == "P3_4" &
		file == "2014_12_18A_full"]
d2$Condition <- "SHIP2"
t2 <- regr(d2, 3, "SHIP2")
d3 <- d[Genotype == "PTEN" & Condition == "siSHIP2" &
		Antagonist == "1uMPI-103" & Component.Name == "P3_4" &
		file == "2014_12_18A_full"]
