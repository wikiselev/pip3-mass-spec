...
get_data_f3 <- function(f){
	...
	d$Golden.Ratio.SD.Tech <- d$Golden.Ratio.SD.Tech*1e3
...
f <- files[1]
d <- get_data_f3(f)
p <- ggplot(d, aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = Condition)) +
	geom_bar(position = "dodge", stat="identity") +
	geom_errorbar(limits, width = 0.25, position=position_dodge(.9)) +
	facet_grid(Genotype ~ .) +
	labs(x = "Time, min", y = "PIP3, pmol") +
	theme_bw()
...
f <- files[3]
d <- get_data_f3(f)
d$Antagonist <- factor(d$Antagonist, levels <-
	c("dmso", "1uMPI-103"))
p <- ggplot(d, aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = Antagonist)) +
	geom_bar(position = "dodge", stat="identity") +
	geom_errorbar(limits, width = 0.25, position=position_dodge(.9)) +
...
d <- get_data_f3(f)
d$Antagonist <- factor(d$Antagonist, levels <-
	c("dmso", "10nMPI-103", "30nMPI-103", "100nMPI-103", "300nMPI-103", "1uMPI-103"))
p <- ggplot(d, aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = Antagonist)) +
	geom_bar(position = "dodge", stat="identity") +
	geom_errorbar(limits, width = 0.25, position=position_dodge(.9)) +
	facet_grid(Genotype ~ Condition, scale="free") +
...
f <- files[7]
d <- get_data_f3(f)
d$Condition <- factor(d$Condition, levels <-
	c("sictrl", "siCIIa", "siCIIb", "siINPP4(A+B)", "siINPP4(A+B)&siCIIa", "siINPP4(A+B)&siCIIb"))
p <- ggplot(d, aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = Condition)) +
	geom_bar(position = "dodge", stat="identity") +
	geom_errorbar(limits, width = 0.25, position=position_dodge(.9)) +
	facet_grid(Genotype ~ .) +
...
d <- get_data_f3(f)
d$Antagonist <- factor(d$Antagonist, levels <-
	c("dmso", "IC50PIKTGX", "IC50x10PIKTGX", "IC50x100PIKTGX", "IC50x1000PIKTGX"))
p <- ggplot(d, aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = Antagonist)) +
	geom_bar(position = "dodge", stat="identity") +
	geom_errorbar(limits, width = 0.25, position=position_dodge(.9)) +
	facet_grid(Genotype ~ Condition, scale="free") +
...
d <- get_data_f34(f)
d$Condition <- factor(d$Condition, levels <-
	c("sictrl", "siINPP4(A+B)", "siSHIP2"))
p <- ggplot(d, aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = Condition)) +
	geom_bar(position = "dodge", stat="identity") +
	geom_errorbar(limits, width = 0.25, position=position_dodge(.9)) +
	facet_grid(Genotype ~ .) +
	labs(x = "Time, min", y = "PI(3,4)P2, pmol") +
	theme_bw()
ggsave(p, file = paste0("../plots/pi34p2/", strsplit(f, "\\.")[[1]][1], ".pdf"), w = 8, h = 8)
d <- get_data_f45(f)
d$Condition <- factor(d$Condition, levels <-
	c("sictrl", "siSHIP2", "siINPP4(A+B)"))
p <- ggplot(d, aes(x = as.factor(Time), y = Golden.Ratio.Av.Tech, fill = Condition)) +
	geom_bar(position = "dodge", stat="identity") +
	geom_errorbar(limits, width = 0.25, position=position_dodge(.9)) +
	facet_grid(Genotype ~ .) +
	labs(x = "Time, min", y = "PI(4,5)P2, pmol") +
	theme_bw()
...
d$Antagonist <- factor(d$Antagonist, levels <-
	c("dmso", "1uMPI-103"))
p <- ggplot(d, aes(x = Time, y = Golden.Ratio.Av.Tech, color = Antagonist, group = Antagonist)) +
	geom_point() +
	geom_line() +
	facet_grid(Genotype ~ Condition) +
	labs(x = "Time, s", y = "PIP3, pmol") +
	theme_bw()
...
p <- ggplot(d, aes(x = Time, y = Golden.Ratio.Av.Tech, color = Antagonist, group = Antagonist)) +
	...
	geom_line() +
	facet_grid(Genotype ~ Condition) +
	labs(x = "Time, s", y = "PI(3,4)P2, pmol") +
	theme_bw()
...
p <- ggplot(d, aes(x = Time, y = Golden.Ratio.Av.Tech, color = Antagonist, group = Antagonist)) +
	...
	geom_line() +
	facet_grid(Genotype ~ Condition) +
	labs(x = "Time, s", y = "PI(4,5)P2, pmol") +
	theme_bw()
